...
export class SupabaseStore implements DataStore {
  ...
  async deleteVariantGroup(id: string): Promise<void> {
    const db = await this.db();
    const existing = await db.from('option_variant_groups').select('id, option_id, code').eq('id', id).maybeSingle();
    ...
    const dependentGroups = await db
      .from('option_variant_groups')
      .select('id', { count: 'exact', head: true })
      .eq('option_id', existing.data.option_id)
      .eq('depends_on_group_code', existing.data.code);
    if (dependentGroups.error) mapPgError(dependentGroups.error);
    if ((dependentGroups.count ?? 0) > 0) {
      throw new StoreError('VALIDATION', '別の選択項目の表示条件に使われているため削除できません。先に表示条件を解除してください。');
    }

    const { error } = await db.from('option_variant_groups').delete().eq('id', id);
    if (error) mapPgError(error);
  }
  async deleteVariantChoice(id: string): Promise<void> {
    ...
    const used = await db
      ...
      .select('id', { count: 'exact', head: true })
      .contains('variant_choice_ids', [id]);
    if (used.error) mapPgError(used.error);
    if ((used.count ?? 0) > 0) {
    ...
    if (parent.data) {
      const dependentGroups = await db
        ...
        .eq('option_id', parent.data.option_id)
    ...
    const { error } = await db.from('option_variant_choices').delete().eq('id', id);
    if (error) mapPgError(error);
  }
