...
function selectionAmount(
  ...
  variantGroups: OptionVariantGroup[],
  variantChoices: OptionVariantChoice[]
): { amount: number; priceOnRequest: boolean } {
  ...
  for (const option of options) {
    ...
    if (option.price_on_request) priceOnRequest = true;
    else amount += option.price;
    ...
    for (const choice of variantChoices) {
      ...
      if (choice.price_on_request) priceOnRequest = true;
      else amount += choice.extra_price;
    }
  }

  return { amount, priceOnRequest };
}
...
export function equipmentCategoryPriceState({
  ...
  variantGroups,
  variantChoices,
}: EquipmentCategoryPriceStateInput): EquipmentCategoryPriceState {
  ...
  if (current.priceOnRequest || standard.priceOnRequest) {
    return { kind: 'price-on-request' };
  }

  const delta = current.amount - standard.amount;
  return delta === 0 ? { kind: 'no-change' } : { kind: 'delta', delta };
}
