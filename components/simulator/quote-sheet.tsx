...
export function QuoteSheet({
  ...
  const standardMobileRows = (code: 'interior_exterior' | 'option' | 'sitework') => {
    const section = standardSection(code);
    if (!section) return null;
    return (
      <>
        {standardLines(code).map((line) => (
          <MobileQuoteLine
            ...
            name={
              <>
                {line.group_label && <span className="mr-1.5 text-[0.7rem] text-muted">{line.group_label}</span>}
                {line.name}
              </>
            }
            quantity={line.quantity == null ? '—' : `${formatQty(line.quantity)}${line.unit ?? ''}`}
            unitPrice={line.unit_price == null ? '—' : formatYen(line.unit_price)}
            amount={formatYen(line.amount)}
        ...
        {section.delta_line !== 0 && (
          <MobileQuoteLine
            name="選択商品の変更差額"
            quantity="1式"
            amount={formatYen(section.delta_line)}
            remark="商品マスターとの差額"
          />
        )}
        {code !== 'sitework' && section.expense_amount !== 0 && (
          <MobileQuoteLine
            ...
            quantity="1式"
            amount={formatYen(section.expense_amount)}
            muted
          />
        )}
      </>
  ...
  return (
    <section aria-labelledby="quote-sheet-heading" className="overflow-hidden border border-line bg-white shadow-soft" data-testid="quote-sheet">
      ...
      <div className="sm:hidden" data-testid="quote-sheet-mobile">
        ...
        />
        {expandedSections.interiorExterior && standardEstimate && standardMobileRows('interior_exterior')}
        {expandedSections.interiorExterior && !standardEstimate && displayInteriorExteriorLines.map((line) => {
        ...
        />
        {expandedSections.options && standardEstimate && standardMobileRows('option')}
        {expandedSections.options && !standardEstimate && optionLines.map((line) => {
          const cat = categories.find((category) => category.id === byOption.get(line.option_id)?.category_id);
          const amountText = line.price_on_request
            ? '別途見積'
            : line.amount === 0
        ...
        {expandedSections.options && !standardEstimate && (
          <MobileQuoteLine
        ...
        />
        {expandedSections.sitework && standardEstimate && standardMobileRows('sitework')}
        {expandedSections.sitework && !standardEstimate && sitework.map((line) => (
          <MobileQuoteLine
            key={line.code}
            name={line.name}
            quantity={`${formatQty(line.quantity)}式`}
