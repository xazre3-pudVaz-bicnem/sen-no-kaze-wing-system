...
export const seedCategories: OptionCategory[] = [
  ...
  cat(C.exteriorWall, 'exterior-wall', '外壁', G.finish, 3, { is_required: true, description: '外壁の仕上げ材' }),
  cat(C.sash, 'sash', 'サッシ', G.sash, 1, { is_required: true }),
  cat(C.interiorDoor, 'interior-door', '内部建具', G.door, 1, { is_required: true }),
  cat(C.ub, 'ub', '浴室（ユニットバス）', G.equipment, 1, { description: '浴室ユニット。いずれか1つ' }),
  cat(C.toilet, 'toilet', 'トイレ', G.equipment, 2, { selection_mode: 'multi' }),
  cat(C.washbasin, 'washbasin', '洗面', G.equipment, 3, { selection_mode: 'multi' }),
  cat(C.kitchen, 'kitchen', 'キッチン', G.equipment, 4, { selection_mode: 'multi' }),
  cat(C.boiler, 'boiler', '給湯', G.equipment, 5, { selection_mode: 'multi' }),
  cat(C.aircon, 'aircon', '空調', G.equipment, 6, { selection_mode: 'multi' }),
  cat(C.lighting, 'lighting', '照明器具', G.lighting, 1, { selection_mode: 'multi' }),
  cat(C.furniture, 'furniture', '家具', G.furniture, 1, { selection_mode: 'multi' }),
  cat(C.appliances, 'appliances', '家電', G.furniture, 2, { selection_mode: 'multi' }),
  cat(C.smartlock, 'smartlock', 'スマートロック・鍵', G.other, 1, { selection_mode: 'multi' }),
  cat(C.carpentry, 'carpentry', '造作工事', G.other, 2, { is_required: true }),
  cat(C.exteriorParts, 'exterior-parts', '外構部品', G.other, 3, { selection_mode: 'multi' }),
  cat(C.officeSupplies, 'office-supplies', '事務所用品', G.other, 4, { selection_mode: 'multi' }),
  cat(C.fireproof, 'fireproof', '防火仕様', G.fireproof, 1, { is_required: true, description: '建築する場所によって異なります。詳しくは近くの代理店にご相談ください' }),
...
export const seedOptions: ProductOption[] = [
  ...
  opt({ id: O.insulationUpgradeWing, base_model_id: MODEL_WING01_ID, category_id: C.smartlock, code: 'insulation-upgrade-wing', name: '高断熱仕様（スタイロフォーム 90mm）', description: '床・壁・天井の断熱材をグラスウールからスタイロフォーム 90mm に変更。', price: 235278, selection_type: 'checkbox', sort_order: 5 }),
