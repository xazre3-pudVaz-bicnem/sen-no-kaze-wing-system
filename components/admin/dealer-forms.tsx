...
export function DealerRevisionForm({
  ...
  const [rows, setRows] = useState<Row[]>(() =>
    items
      .filter((i) => i.kind === 'installation' || i.kind === 'free')
      .map((i, n) => ({
        key: `${i.id}-${n}`,
        kind: i.kind === 'free' ? 'free' : 'installation',
        name: i.name,
  ...
  const entered = rows.reduce((s, r) => s + r.unit_price * Math.max(1, r.quantity), 0);
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[44rem] text-sm">
          <thead className="bg-sand/60 text-left text-xs text-muted">
            <tr>
              <th className="w-24 px-3 py-2 font-semibold">区分</th>
            ...
            {rows.map((r, i) => (
              <tr key={r.key} data-testid={`revision-row-${i}`}>
                <td className="px-3 py-2">
                  ...
                  <span className={r.kind === 'free' ? 'text-brown' : 'text-muted'}>{r.kind === 'free' ? 'フリー商品' : '別途工事'}</span>
                </td>
                <td className="px-3 py-2">
                  <Input
                    name={`items.${i}.name`}
