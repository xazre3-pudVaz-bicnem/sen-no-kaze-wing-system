...
begin
  if exists (
    ...
    where cfg.status = 'draft'
      and cat.code = 'exterior-wall'
    group by cfg.id
    having count(*) > 1
  ) then
    raise exception
      'MIGRATION_BLOCKED: 1つのDraftに複数の旧外壁商品が保存されています。手動確認後に移行してください'
      using errcode = 'P0001';
  end if;
end
$;
...
with legacy_wall as (
  select
    ...
    ci.option_id,
    coalesce(ci.variant_choice_ids, '{}'::uuid[]) as variant_choice_ids
  from public.configurations cfg
  join public.configuration_items ci on ci.configuration_id = cfg.id
  join public.options o on o.id = ci.option_id
  join public.option_categories cat on cat.id = o.category_id
  where cfg.status = 'draft'
    and cfg.exterior_faces = '[]'::jsonb
    and cat.code = 'exterior-wall'
)
...
set exterior_faces = jsonb_build_array(
  jsonb_build_object(
    'face_code', 'front',
    'option_id', lw.option_id,
    'variant_choice_ids', to_jsonb(lw.variant_choice_ids)
  ),
  jsonb_build_object(
    'face_code', 'right',
    'option_id', lw.option_id,
...
from legacy_wall lw
where cfg.id = lw.configuration_id;
