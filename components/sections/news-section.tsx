...
export function NewsSection({ limit = 4 }: { limit?: number }) {
  return (
    <section id="news" className="scroll-mt-20 bg-ivory py-20 sm:py-28">
      <div className="container-x grid gap-12 lg:grid-cols-[1fr_2fr]">
        ...
        <ul className="divide-y divide-line border-y border-line">
          {news.slice(0, limit).map((n) => (
            <li key={n.slug}>
              <Link href={`/news/${n.slug}`} className="group flex flex-wrap items-baseline gap-x-5 gap-y-1 py-5 transition-colors hover:text-brown">
                <time dateTime={n.date} className="text-sm tabular-nums text-muted">
                  {formatDate(n.date)}
                </time>
                <span className="label-en text-gold">{n.category}</span>
                <span className="w-full font-serif text-lg sm:w-auto sm:flex-1">{n.title}</span>
