...
export function ProductLedgerClient({ canEdit, categories, options, variantsByOptionId, initiallySelectedId }: Props) {
  ...
  return <div className="space-y-5">
    <section className="card overflow-hidden">
      ...
      <div className="grid gap-2 border-b border-line bg-white p-3 sm:grid-cols-[minmax(16rem,1fr)_minmax(10rem,.42fr)_minmax(9rem,.34fr)_minmax(11rem,.42fr)] sm:p-4">
        <label className="min-w-0">
          <span className="sr-only">商品を検索</span>
          <Input type="search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="商品名・メーカー・型番を検索" className="w-full" />
        </label>
        <label className="min-w-0">
