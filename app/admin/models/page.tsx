...
export default async function AdminModelsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return (
    <AdminPage title="ベースコンテナ" lead="商品ごとに基本情報・プラン・商品画像・シミュレーター画像をまとめて管理します。" actions={<Link href="/admin/models/new" className="btn-primary btn-sm">新規作成</Link>}>
      <FlashMessages sp={sp} />
      <Table>
        ...
        <tbody className="divide-y divide-line">
          {models.map((m) => (
            <tr key={m.id}>
              ...
              <Td right><Link href={`/admin/models/${m.id}`} className="btn-secondary btn-sm">管理</Link></Td>
            </tr>
          ))}
        </tbody>
      </Table>
