...
describe('商品管理番号の自動採番', () => {
  ...
  it('ローカル検証モードでも採番高水位を保持し、削除後に番号を再利用しない', () => {
    expect(localDb).toContain('productNoSequence: number');
    expect(localDb).toContain('db.productNoSequence + 1');
    expect(localDb).toContain('db.productNoSequence = next');
    expect(localDb).toContain('db.productNoSequence < maxExisting');
    expect(localDb).toContain('商品管理番号「${option.product_no}」の形式が正しくありません');
